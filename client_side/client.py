...
class Dialing(Frame):
    ...
    def answer(self, timeout=1):
        max_time = time.time() + 60 * timeout  # 1 minutes from start
    ...
    def dialing(self):
        ...
        if is_posted:
            ...
            if result == 'accepted':
                ...
                self.controller.show_frame(Main)
                if result == 'timed_out':  # waited too long -one minute for response from the call target
...
class Register(Frame):
    ...
    def handle(self, event=None):
        ...
        # adds to database unless name is already used
        else:
